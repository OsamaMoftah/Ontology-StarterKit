...
| U03 | Implemented / verified | Universal and Python 3.10/3.11/3.12 core/optional locks are generated and byte-checked by `scripts/regenerate_locks.py`; CI installs the matching versioned lock. The generic Python 3.11 locks are retained only for legacy compatibility and are not the canonical freshness-checked contract. | Regenerate when dependency inputs change. |
...
| U20 | Implemented / verified | LinkML identifiers, required slots, enums, non-negative cardinalities, min/max ordering, scalar versus collection rendering including maximum zero, and independent valid/cardinality/enum fixtures are covered by runtime tests; the clean locked optional environment passes `gen-json-schema --top-class Person --closed` with explicit `maxItems` and `enum` assertions. | Review conversion losses before using generated JSON Schema as a production contract. |
...
- `PATH=/tmp/ontokit-linkml-clean/bin:$PATH python3 scripts/verify_linkml_generator.py`: LinkML 1.11.1 generated a closed JSON Schema that accepted the valid fixture, independently rejected the cardinality-only and enum-only fixtures, and enforced maximum-zero collection semantics.
- `uv pip sync requirements/optional-py311.lock` in a fresh `/tmp` environment installed the locked optional packages and exposed `gen-json-schema` 1.11.1.
