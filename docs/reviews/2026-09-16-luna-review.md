...
| U03 | Implemented / verified | Universal core and optional locks are generated and byte-checked by `scripts/regenerate_locks.py` for the supported Python/platform marker set. | Regenerate when dependency inputs change. |
...
| U22 | Implemented / verified | Real MCP stdio tests cover oversized responses, timeout boundary, malformed args, unknown query, and symlink escape; spawned-worker timeout tests and the 16-iteration benchmark show no accumulation. | Tune limits for a deployment's workload. |
