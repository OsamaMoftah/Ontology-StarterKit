...
| --- | --- | --- | --- |
| U01 | Partial | `src/ontology_starterkit/neo4j_queries.py` has allowlisted parameterized templates; generated Cypher remains opt-in in `graph_rag.py`; local Neo4j parity is proven by `scripts/verify_neo4j_runtime.py`. | Database-enforced read-only account on Enterprise or an equivalent service. |
| U02 | Partial | `neo4j_runtime.py` uses an explicit server transaction timeout; the live verifier proves termination and a post-cancel heartbeat. MCP calls enforce types, deadlines, row and byte limits; retries are bounded. | Process-level worker accumulation benchmark for expensive local MCP queries. |
...
| U11 | Partial | Seed imports preserve literal datatype/language and blank-node identity, remove stale scoped relationships, and remain idempotent; the live verifier proves RDFLib-to-Neo4j parity and Compose health. | Community image cannot grant a database reader role; Enterprise or equivalent access-control proof remains blocked. |
| U12 | Partial | `AnswerRecord` and `assess_answer` separate citation membership, graph-path validity, source review, and answer correctness; the reviewed fixture covers supported, unsupported, conflicting, stale, and path-invalid outcomes. | Source-document span offset verification over a domain-reviewed evaluation set. |
...
| U16 | Partial | Issue form/config, citation/reporting guidance, clean wheel check, and renamed-clone validation are present. | Hosted GitHub template settings and issue-form behavior still require repository-owner verification. |
| U17 | Partial | Just targets now include lesson and runtime verification; full optional lock includes the LinkML generator; Compose startup, seed, runtime parity/cancellation, and teardown pass locally. | Enterprise access-control proof and optional-service acceptance on CI. |
...
| U20 | Implemented / verified | LinkML identifiers, required slots, cardinalities, enums, and valid/invalid fixtures are covered by runtime tests; `requirements/optional.lock` pins `linkml`, and `scripts/verify_linkml_generator.py` passes `gen-json-schema --top-class Person --closed`. | Review conversion losses before using generated JSON Schema as a production contract. |
...
| U22 | Partial | Real MCP stdio tests cover oversized responses, timeout boundary, malformed args, unknown query, and symlink escape; live Neo4j cancellation is proven by the separate runtime verifier. | Process-level isolation and worker accumulation benchmark for expensive local MCP queries. |
...
| U24 | Partial | All 12 engagement cards include sponsor, users, decision, sources, slice, deliverable, measures, review, and boundaries; seven named operational templates are committed and linked. | Add client-ready sample records and a final consultancy-owner review. |
...
The separate access-control check remains externally blocked by the pinned Community image: `GRANT ROLE reader TO starterkit_reader` returns `Neo.ClientError.Statement.UnsupportedAdministrationCommand`. The verifier fails closed rather than treating an admin account as proof of database-enforced read-only access.
